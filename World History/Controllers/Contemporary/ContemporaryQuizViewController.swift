//
//  ContemporaryQuizViewController.swift
//  World History
//
...
class ContemporaryQuizViewController: UIViewController {
  ...
  var contemporaryQuizBrain = ContemporaryQuizBrain()
  ...
  override func viewDidLoad() {
    ...
    loadNextQuestion()
    addContemporaryQuizStartOverlay()
  }
  ...
  func loadNextQuestion() {
    ...
    progressBar.progress = Float(currentQuestion + 1) / Float(contemporaryQuizBrain.questions.count)
  }
  
  
  func checkAnswer(_ sender: UIButton) {
    let correctAnswerIndex = contemporaryQuizBrain.questions[currentQuestion].correctAnswerIndex
  ...
  func showEndQuizAlert() {
    let resultInPercentage = Int(round(Double(score) / Double(contemporaryQuizBrain.questions.count) * 100.0))
  ...
  @objc func resetQuiz() {
    currentQuestion = 0
    score = 0
    contemporaryQuizBrain = ContemporaryQuizBrain()
    loadNextQuestion()
  }
...
extension UIViewController {
  func addContemporaryQuizStartOverlay() {
  ...
  @objc func removeContemporaryQuizStartOverlay() {
